variable "kubeconfig_path" {
  ...
  validation {
    condition     = var.kubeconfig_path == null || fileexists(pathexpand(var.kubeconfig_path))
    error_message = "kubeconfig_path must point to an existing file. Use the kubeconfig generated by components/kind-cluster or set KUBECONFIG to a valid path."
  }
}

variable "cluster_name" {
  type        = string
  description = "Cluster name used to derive the default kubeconfig path when kubeconfig_path is null."
  default     = "blitzinfra"
}
