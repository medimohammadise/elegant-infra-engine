...
- Current component roots include `infra`, `postgres`, `kind-cluster`, `backstage`, `headlamp`, `kafka`, `keycloak`, and `observability`. Output kubeconfigs live under `components/kubeconfigs`.
...
- Preserve idempotency and support partial deployments where a component may be applied independently from `components/infra` and the app roots.
- Keep the common `components/infra` outputs aligned with any standalone component roots that depend on the shared infrastructure, especially for Keycloak and observability.
...
- When exposing Grafana or Prometheus publicly, make sure the matching `kind` host-port mappings stay wired through `modules/kind-cluster` and the infra outputs from `components/infra` remain in sync.
...
- For `components/infra` plans that touch the remote `kind` cluster, use the Docker environment expected by the repo so Terraform can talk to the remote daemon through the `kind` provider.
- For observability changes in `components/observability`, confirm the plan does not accidentally destroy the live `module.observability` resources when only documentation or variable wiring was intended to change.
