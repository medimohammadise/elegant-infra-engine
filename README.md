...
Before applying, make sure `kubeconfig_path` points to a real kubeconfig file (for this repo, usually `../kind-cluster/blitzinfra-kubeconfig` generated by `components/kind-cluster`). If the file is missing, Terraform will fail provider initialization.
...
- **kubeconfig_path refers to an invalid path**
  - Run `components/kind-cluster` first, or set `kubeconfig_path` to an existing kubeconfig file.
  - Use a path that exists from inside `components/observability` (for example `../kind-cluster/blitzinfra-kubeconfig`).
  - You can also point `KUBECONFIG` to a valid file and mirror that path in `terraform.tfvars`.
- **Kibana unavailable**
